...
def run_training(d_model: int = 512, nhead: int = 8, num_encoder_layers: int = 6, num_decoder_layers: int = 6,
    ...
    for epoch in range(epochs):
        ...
        for src_batch, tgt_batch in zip(src, tgt):
            ...
            tgt_mask = tgt_mask[:, :-1]
            output = model.forward(src_input, src_key_mask=src_mask, tgt=decoder_input, tgt_key_mask=tgt_mask)
...
if __name__ == "__main__":
    run_training(d_model=512, dim_feedforward=512, num_decoder_layers=6, num_encoder_layers=6, batch_size=16)
